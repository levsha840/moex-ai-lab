...
class StrategyCatalog:
    def get_active_strategies(self):
        ...
        try:
            return pd.read_sql("""
                SELECT id AS strategy_catalog_id, strategy_name, ticker, version, status
    ...
    def update_replay_stats(self, stats):
        conn = get_connection()
        cur = conn.cursor()
        try:
            for _, row in stats.iterrows():
                cur.execute("""
                    ...
                    SET last_replay=now(),
                        ...
                        replay_total_pnl=%s,
                        updated_at=now()
                    WHERE strategy_name=%s AND ticker=%s AND status='active';
                """, (
